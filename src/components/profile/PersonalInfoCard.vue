        ...
        <div>
          ...
          <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
            <div>
              ...
              <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ nombre || '—' }}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ telefono || '—' }}</p>
            </div>
            <div>
    ...
    <Modal v-if="isProfileInfoModal" @close="isProfileInfoModal = false">
      <template #body>
        <div class="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          ...
          >
            <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none"><path fill-rule="evenodd" clip-rule="evenodd" d="M6.04289 16.5418C5.65237 16.9323 5.65237 17.5655 6.04289 17.956C6.43342 18.3465 7.06658 18.3465 7.45711 17.956L11.9987 13.4144L16.5408 17.9565C16.9313 18.347 17.5645 18.347 17.955 17.9565C18.3455 17.566 18.3455 16.9328 17.955 16.5423L13.4129 12.0002L17.955 7.45808C18.3455 7.06756 18.3455 6.43439 17.955 6.04387C17.5645 5.65335 16.9313 5.65335 16.5408 6.04387L11.9987 10.586L7.45711 6.04439C7.06658 5.65386 6.43342 5.65386 6.04289 6.04439C5.65237 6.43491 5.65237 7.06808 6.04289 7.4586L10.5845 12.0002L6.04289 16.5418Z" fill="" /></svg>
          </button>
          <div class="px-2 pr-14">
            <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">Editar información personal</h4>
            <p class="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">Actualiza tus datos. El email no se puede cambiar.</p>
          </div>
...
const emit = defineEmits<{
  save: [payload: { nombre?: string; telefono?: string; bio?: string }]
}>()

const isProfileInfoModal = ref(false)
const form = ref({ nombre: '', telefono: '', bio: '' })
...
watch(() => [props.nombre, props.telefono, props.bio], () => {
  if (isProfileInfoModal.value) {
    form.value = {
      nombre: props.nombre ?? '',
      telefono: props.telefono ?? '',
      bio: props.bio ?? '',
